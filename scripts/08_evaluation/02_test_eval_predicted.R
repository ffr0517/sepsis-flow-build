...
variant_results_dir <- file.path(save_dir, "prior_variant_results_D1_PREDICTED")
...
replace_day2_day1_treatments_with_predictions <- function(day2_input, pred_day1_long) {
  ...
  assert_required_columns(
    pred_day1_for_day2,
    c("patient_index", d1_cols),
    "Day 1 predictions reshaped for Day 2 input"
  )
  ...
  out <- day2_input
  for (col in d1_cols) {
    out[[col]] <- pred_day1_for_day2[[col]]
  }
...
build_final_df_for_variant <- function(raw_test, variant_row) {
  ...
  day2_input_with_pred_d1 <- replace_day2_day1_treatments_with_predictions(
    day2_input = inputs$day2_input,
    pred_day1_long = pred_day1_long
  )
  pred_day2_long <- call_day_api(day = 2, input_df = day2_input_with_pred_d1, variant_id = variant_row$variant_id)
...
# Orchestration ----
# Examples:
# Rscript test_eval_2.R --mode=run --variants=whole_prior
# Rscript test_eval_2.R --mode=run --variants=country_prior
# Rscript test_eval_2.R --mode=run --variants=inpatient_status_prior
# Rscript test_eval_2.R --mode=run --variants=country_x_inpatient_prior
# Rscript test_eval_2.R --mode=run --variants=normal

# Rscript test_eval_2.R --mode=combine --variants=saved
