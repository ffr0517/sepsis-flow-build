...
SELECTED_OUTCOME_VAR <- "LEVEL5_TREATMENTS_D1_SAFE_0"
...
run_spline_lasso_one <- function(
  ...
  # 7. Variable Importance (Corrected)
  vip_metric_fn <- switch(
    vi_metric,
    roc_auc = function(truth, estimate) {
      yardstick::roc_auc_vec(truth = truth, estimate = estimate, event_level = "second")
  ...
  # 2. Standardize the Standard Deviation column name
  # vip::vi_permute usually returns "StDev", but older versions might use "StdError"
  if ("StDev" %in% names(df_x_varimp_raw)) {
    df_x_varimp_raw <- df_x_varimp_raw %>% rename(StDev = StDev)
  }
  
  # If for some reason neither exists (e.g. nsim=1), ensure StDev exists as NA
  if (!"StDev" %in% names(df_x_varimp_raw)) {
    df_x_varimp_raw$StDev <- NA_real_
  }
  
  # 3. Aggregate Spline Terms
  df_x_varimp <- df_x_varimp_raw %>%
    mutate(
      # Group spline terms (e.g., age_ns_1, age_ns_2 -> age)
...
# building predicted treatment df
df3 <- df2

df3[[paste0("PRED_", SELECTED_OUTCOME_VAR)]] <- df2_votes_unajd$vote_class
...
saveRDS(df3, 
        file.path(save_dir, paste0(SELECTED_OUTCOME_VAR, "_pred_training_data.rds")))

# Evaluating on TSET data (probability-based, no thresholds) ----
save_dir <- PATHS$ensemble_workspace

heads <- readRDS(file.path(save_dir, paste0(SELECTED_OUTCOME_VAR, "_ensemble.rds"))
)

raw_test <- raw_test %>%
  left_join(measures_df, by = c("label" = "Label"))

raw_test[[SELECTED_OUTCOME_VAR]] <- factor(
  raw_test[[SELECTED_OUTCOME_VAR]],
  levels = c(0, 1),
  labels = c("no", "yes")
)

# Per-head probabilities only
raw_test_preds_all <- imap_dfc(heads, function(h, name) {
  p <- predict(h$model, new_data = raw_test, type = "prob")$.pred_yes
  tibble(!!paste0("p_", name) := p)
}) %>%
  bind_cols(raw_test %>% select(.data[[SELECTED_OUTCOME_VAR]]))

# Ensemble probability summaries
raw_test_probs <- raw_test_preds_all %>%
  mutate(
    p_mean = rowMeans(across(starts_with("p_")), na.rm = TRUE),
    p_median = apply(select(., starts_with("p_")), 1, median, na.rm = TRUE),
    p_min = apply(select(., starts_with("p_")), 1, min, na.rm = TRUE),
...
raw_test_votes_unajd <- raw_test_preds_all %>%
  mutate(
    n_yes = rowSums(across(starts_with("p_"), ~ .x >= 0.5), na.rm = TRUE),
    n_no = rowSums(across(starts_with("p_"), ~ .x < 0.5), na.rm = TRUE),
    vote_class = factor(
...
training_data_mean_pred_perf <- raw_test_probs %>%
  group_by(.data[[SELECTED_OUTCOME_VAR]]) %>%
  summarise(
    mean_p_mean = mean(p_mean, na.rm = TRUE),
    sd_p_mean = sd(p_mean, na.rm = TRUE),
...
ggplot(raw_test_probs, aes(x = p_mean, fill = as.factor(.data[[SELECTED_OUTCOME_VAR]]))) +
  geom_density(alpha = 0.5) +
  labs(
    title = "Distribution of Mean Prediction by Treatment",
    subtitle = "Full training data",
