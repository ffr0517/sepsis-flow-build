...
extract_preprocess_constants_type2 <- function(head_path) {
  ...
  model_min <- strip_model_type2_glmnetpredict(head_path)
  ...
  for (st in ns_steps) {
    ...
    for (var in names(objs)) {
      ...
      expected_cols <- grep(paste0("^", var_escaped, "_ns_"), features, value = TRUE)
...
# --- Day 2 Level 5 (non-bolused IV fluids) ----
# Type 1 
build_type1_bundle_1to40 <- function(
    head_dir = file.path(PATHS$outputs_exports, "sepsis-flow-D2-L5", "models"),
    head_indices = 1:40,
    head_prefix = "head_",
    head_suffix = ".rds"
) {
  ...
  feature_signatures <- vapply(
    models,
    function(m) paste(m$features, collapse = "|"),
    character(1)
  )
  unique_signatures <- unique(feature_signatures)
  if (length(unique_signatures) > 1) {
    mismatch_idx <- head_indices[feature_signatures != feature_signatures[[1]]]
    message(
      "Type 1 note: detected ", length(unique_signatures),
      " feature signatures across heads. Continuing. Heads differing from first: ",
      paste(mismatch_idx, collapse = ", ")
    )
  }

  # Keep a union of features for metadata/debugging; per-head prediction uses each model's own features.
  union_features <- sort(unique(unlist(lapply(models, function(m) m$features), use.names = FALSE)))
...
build_type2_bundle_41to80 <- function(
  ...
  if (length(unique_signatures) > 1) {
    mismatch_idx <- head_indices[feature_signatures != feature_signatures[[1]]]
    message(
      "Type 2 note: detected ", length(unique_signatures),
      " feature signatures across heads. Continuing. Heads differing from first: ",
      paste(mismatch_idx, collapse = ", ")
    )
  }
...
# Saving joint bundle 
ensemble1_path <- "sepsis-flow-D2-L5/models/ensemble1_bundle.rds"
...
# --- Day 2 Level 4 (O2 via face or nasal cannula) ----
# Type 1 
build_type1_bundle_1to40 <- function(
    head_dir = file.path(PATHS$outputs_exports, "sepsis-flow-D2-L4", "models"),
    head_indices = 1:40,
    head_prefix = "head_",
    head_suffix = ".rds"
) {
...
# Saving joint bundle 
ensemble2_path <- "sepsis-flow-D2-L4/models/ensemble2_bundle.rds"
...
# --- Day 2 Level 3 (ICU admission with clinical reason) ----
# Type 1 
build_type1_bundle_1to40 <- function(
    head_dir = file.path(PATHS$outputs_exports, "sepsis-flow-D2-L3", "models"),
    head_indices = 1:40,
    head_prefix = "head_",
    head_suffix = ".rds"
) {
...
# Saving joint bundle 
ensemble3_path <- "sepsis-flow-D2-L3/models/ensemble3_bundle.rds"
...
# --- Day 2 Level 2 (CPAP or IV fluid bolus) ----
# Type 1 
build_type1_bundle_1to40 <- function(
    head_dir = file.path(PATHS$outputs_exports, "sepsis-flow-D2-L2", "models"),
    head_indices = 1:40,
    head_prefix = "head_",
    head_suffix = ".rds"
) {
...
# Saving joint bundle 
ensemble4_path <- "sepsis-flow-D2-L2/models/ensemble4_bundle.rds"
...
# --- Day 2 Level 1 (mechanical ventilation, inotropes, or renal replacement therapy) ----
# Type 1 
build_type1_bundle_1to40 <- function(
    head_dir = file.path(PATHS$outputs_exports, "sepsis-flow-D2-L1", "models"),
    head_indices = 1:40,
    head_prefix = "head_",
    head_suffix = ".rds"
) {
...
manual_type3_matrix <- function(new_data, preprocess) {
  ...
  base_vars <- c(
    ...
    "not.alert", "hr.all", "rr.all", "envhtemp", "crt.long", "oxy.ra",
    "LEVEL1_TREATMENTS_D1_SAFE_0", "LEVEL2_TREATMENTS_D1_SAFE_0",
    "LEVEL2_TREATMENTS_D1_SAFE_0", "LEVEL2_TREATMENTS_D1_SAFE_0",
...
# Saving joint bundle 
ensemble5_path <- "sepsis-flow-D2-L1/models/ensemble5_bundle.rds"
...
# Saving day 2 treatment bundles ----
bundle_day2 <- list(
  kind = "day2_bundle",
  created_utc = format(Sys.time(), tz = "UTC"),
  levels = list(
...
dir.create(dirname(file.path(PATHS$outputs_exports, "day2_bundle.rds")), recursive = TRUE, showWarnings = FALSE)
saveRDS(bundle_day2, file.path(PATHS$outputs_exports, "day2_bundle.rds"), compress = "xz")
